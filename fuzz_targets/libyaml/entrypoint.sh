...
: "${FUZZER_ID:=}"
# Path to your libyaml fuzzer binary
: "${FUZZER_BIN:=/out/tokener_parse_ex_fuzzer}" # there are four of them, this is one of them
if [[ -f "/out/tokener_parse_ex_fuzzer.dict" ]]; then
  AFL_DICT="/out/tokener_parse_ex_fuzzer.dict"
  DICT_ARG=(-x "$AFL_DICT")
else
  echo "No dictionary found, running without it."
  DICT_ARG=()
fi
